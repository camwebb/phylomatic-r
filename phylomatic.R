...
phylomatic <- function(phylo, taxa) {
    ...
    if(length(phylo$edge.length) == 0) {
        ...
        ## missing BLs in existing $edge.length are NaN
        ## NA could be used, but C deals with NA as NaN anyway
        BL <- 0
    } else {
        BL <- 1
    }
    ...
    ## you can reference output sub-objects by out[[12]] or $out_from
    ...
    ape <- list(Nnode = x$out_nnode,
                ...
                tip.label = x$out_tip_lab[1:x$out_ntip])
